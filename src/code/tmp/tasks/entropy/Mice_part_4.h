#ifndef _MICE_PART_4_H_
#define _MICE_PART_4_H_

#include "param.h"

template <int init_mem_in_bytes> class MicePart_4 {
      ...
      public:
        ...
        void insert(uint8_t *key, int f = 1) {
                uint32_t hash_val =
                    (uint32_t)bobhash->run((const char *)key, KEY_LENGTH_4);
      ...
      public:
        void compress(int ratio, uint8_t *dst) {
                ...
                for (int i = 0; i < width && i < counter_num; ++i) {
                        ...
                        for (int j = i; j < counter_num; j += width)
                                max_val = counters[j] > max_val ? counters[j]
                                                                : max_val;
                        dst[i] = max_val;
                }
        }
        ...
        int query_compressed_part(uint8_t *key, uint8_t *compress_part,
                ...
                uint32_t hash_val =
                    (uint32_t)bobhash->run((const char *)key, KEY_LENGTH_4);
                uint32_t pos =
                    (hash_val % (uint32_t)counter_num) % compress_counter_num;
