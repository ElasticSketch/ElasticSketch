...
#if !defined(TUNNEL_OPT)
    // TODO : following assignments will not be needed once we add the support for encap_replica_from_rid action in switchapi
    modify_field(tunnel_metadata.tunnel_index, tunnel_index);
    modify_field(tunnel_metadata.egress_tunnel_type, tunnel_type);
    modify_field(tunnel_metadata.egress_header_count, header_count);
    modify_field(tunnel_metadata.tunnel_dmac_index, dmac_idx);
...
action set_egress_ifindex_from_rid(egress_ifindex) {
    ...
    defined(ENT_FIN_SPINE_PROFILE) || \
    defined(ENT_FIN_LEAF_PROFILE)
@pragma stage 1
#endif
