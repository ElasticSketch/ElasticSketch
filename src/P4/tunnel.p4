...
table egress_vni {
    ...
    (!defined(MIRROR_NEXTHOP_DISABLE) && \
        !defined(TELEMETRY_REPORT_ENABLE))
/*****************************************************************************/
...
#endif /* !TUNNEL_DISABLE || !MIRROR_NEXTHOP_DISABLE && !TELEMETRY_REPORT_ENABLE */
...
#endif /* IPV6_DISABLE */
#endif /* !TUNNEL_DISABLE || !MIRROR_NEXTHOP_DISABLE*/

#ifndef MIRROR_DISABLE
action f_insert_erspan_common_header() {
    // temp measure: copy_header must be removed when egress parser can skip
    // entire header for mirrored copy
    copy_header(inner_ethernet, ethernet);
    add_header(gre);
...
#endif /* IPV6_DISABLE */
#endif /* MIRROR_NEXTHOP_DISABLE */
#endif /* MIRROR_DISABLE */

#ifdef TELEMETRY_REPORT_ENABLE
action f_insert_telemetry_report_header() {
    // temp measure: copy_header must be removed when egress parser can skip
    // entire header for mirrored copy
    copy_header(inner_ethernet, ethernet);
    add_header(udp);
    add_header(telemetry_report_header);
...
//    modify_field(l3_metadata.egress_l4_sport, hash_metadata.entropy_hash);
    ...
    modify_field(udp.checksum, 0);
#ifdef TELEMETRY_REPORT_ENABLE
    // UDP (8) + telemetry_report (12)
    // Inner Ethernet is already counted in eg_intr_md.pkt_length
    add(udp.length_, eg_intr_md.pkt_length, 20);
#else
    // UDP (8) + telemetry_report (12) + Ethernet (14)
    add(udp.length_, egress_metadata.payload_length, 34);
#endif

    modify_field(telemetry_report_header.version, 0);
    modify_field(telemetry_report_header.next_proto, 0);
    modify_field(telemetry_report_header.dropped, 0);
    modify_field(telemetry_report_header.congested_queue, 0);
    modify_field(telemetry_report_header.path_tracking_flow, 0);
    modify_field(telemetry_report_header.reserved1, 0);
    modify_field(telemetry_report_header.reserved2, 0);
    modify_field(telemetry_report_header.hw_id, 0);
    modify_field(telemetry_report_header.sequence_number, 0);
    modify_field(telemetry_report_header.timestamp,
                 i2e_metadata.ingress_tstamp);
}

action ipv4_udp_telemetry_report_rewrite() {
    f_insert_telemetry_report_header();
    f_insert_ipv4_header(IP_PROTOCOLS_UDP);
    modify_field(ethernet.etherType, ETHERTYPE_IPV4);
#ifdef TELEMETRY_REPORT_ENABLE
    // IPv4 (20) + UDP (8) + telemetry_report (12)
    // Inner Ethernet is already counted in eg_intr_md.pkt_length
    add(ipv4.totalLen, eg_intr_md.pkt_length, 40);
#else
    // IPv4 (20) + UDP (8) + telemetry_report (12) + Ethernet (14)
    add(ipv4.totalLen, egress_metadata.payload_length, 54);
#endif
}
...
table tunnel_encap_process_outer {
    ...
    actions {
        ...
        fabric_rewrite;
#endif /* EGRESS_TUNNEL_CPU_DISABLE */
#ifdef SRV6_ENABLE
        srv6_rewrite;
#endif /* SRV6_ENABLE */
#ifndef TUNNEL_DISABLE
...
#endif /* MPLS_DISABLE */
#endif /* TUNNEL_DISABLE */
#if !defined(MIRROR_NEXTHOP_DISABLE) && !defined(MIRROR_DISABLE)
...
#endif /* MIRROR_NEXTHOP_DISABLE */
#ifdef TELEMETRY_REPORT_ENABLE
        ipv4_udp_telemetry_report_rewrite;
#endif /* TELEMETRY_REPORT_ENABLE */
    }
    size : TUNNEL_HEADER_TABLE_SIZE;
}
...
#ifdef MPLS_UDP_ENABLE
action set_mpls_udp_rewrite_push0(smac_idx, sip_index, dip_index) {
#ifndef TUNNEL_OPT
    modify_field(tunnel_metadata.tunnel_smac_index, smac_idx);
#endif /* TUNNEL_OPT */
    set_ip_index(sip_index, dip_index);
...
action set_mpls_udp_rewrite_push1(label1, exp1, ttl1, smac_idx, sip_index, dip_index, bos) {
    set_ip_index(sip_index, dip_index);
#ifndef TUNNEL_OPT
    modify_field(tunnel_metadata.tunnel_smac_index, smac_idx);
#endif /* TUNNEL_OPT */
...
action set_mpls_udp_rewrite_push2(label1, exp1, ttl1, label2, exp2, ttl2,
                              smac_idx, sip_index, dip_index, bos) {
    set_ip_index(sip_index, dip_index);
#ifndef TUNNEL_OPT
    modify_field(tunnel_metadata.tunnel_smac_index, smac_idx);
#endif /* TUNNEL_OPT */
...
action set_mpls_udp_rewrite_push3(label1, exp1, ttl1, label2, exp2, ttl2,
                              label3, exp3, ttl3, smac_idx, sip_index, dip_index, bos) {
   set_ip_index(sip_index, dip_index);
#ifndef TUNNEL_OPT
   modify_field(tunnel_metadata.tunnel_smac_index, smac_idx);
#endif /* TUNNEL_OPT */
...
control process_tunnel_encap {
    ...
    (!defined(MIRROR_NEXTHOP_DISABLE) && \
        !defined(TELEMETRY_REPORT_ENABLE) )
        /* tunnel rewrites */
#ifndef EGRESS_TUNNEL_CPU_DISABLE
...
#endif /* !TUNNEL_DISABLE || !MIRROR_NEXTHOP_DISABLE && !INT && !POSTCARD && !MOD */
...
#endif /* !TUNNEL_NEXTHOP_ENABLE && !MULTICAST_DISABLE */
